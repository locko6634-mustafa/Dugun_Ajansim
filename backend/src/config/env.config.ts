...
const envSchema = z
  .object({
    ...
    WHATSAPP_MODE: z.enum(["disabled", "cloud-api"]).default("disabled"),
    ...
    WHATSAPP_APP_SECRET: z
      .string()
      .regex(/^$|^[a-fA-F0-9]{32,128}$/)
      .default(""),
    WHATSAPP_WEBHOOK_VERIFY_TOKEN: z
      .string()
      .regex(/^$|^[A-Za-z0-9_-]{32,256}$/)
      .default(""),
    WHATSAPP_TEMPLATE_APPLICATION_APPROVED: z
      .string()
      .regex(/^$|^[a-z0-9_]{1,512}$/)
      .default(""),
    WHATSAPP_TEMPLATE_APPLICATION_REJECTED: z
      .string()
      .regex(/^$|^[a-z0-9_]{1,512}$/)
      .default(""),
    WHATSAPP_TEMPLATE_ACCOUNT_ACTIVATION: z
      .string()
      .regex(/^$|^[a-z0-9_]{1,512}$/)
      .default(""),
  ...
  .superRefine((environment, context) => {
    ...
    if (environment.WHATSAPP_MODE === "cloud-api") {
      const required = [
        "WHATSAPP_APP_ID",
        ...
        "WHATSAPP_TEMPLATE_APPLICATION_APPROVED",
        "WHATSAPP_TEMPLATE_APPLICATION_REJECTED",
        "WHATSAPP_TEMPLATE_ACCOUNT_ACTIVATION"
      ] as const;
      for (const key of required) {
        if (!environment[key])
          context.addIssue({
            code: z.ZodIssueCode.custom,
            path: [key],
            message: `${key} WhatsApp Cloud API için zorunludur`
          });
      }
    }
