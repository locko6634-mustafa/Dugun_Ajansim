...
if (username.length < 3 || !strongPasswordSchema.safeParse(password).success) {
    throw new Error('ADMIN_BOOTSTRAP_USERNAME ve 15-128 karakterlik güvenli ADMIN_BOOTSTRAP_PASSWORD zorunludur.');
}
    ...
    await prisma.$transaction(async (transaction) => {
        ...
        const created = await transaction.user.create({
            data: {
                ...
                activeAt: now,
                temporaryPasswordExpiresAt: createTemporaryPasswordExpiry(env.TEMPORARY_PASSWORD_TTL_HOURS, now),
            },
        });
