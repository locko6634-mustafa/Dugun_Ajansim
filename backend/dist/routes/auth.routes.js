...
import { authenticate, clearAuthCookies, csrfCookieOptions, CSRF_COOKIE_NAME, getSessionAbsoluteTtlMs, isTemporaryPasswordExpired, sessionCookieOptions, verifyCsrf, } from '../middlewares/auth.middleware.js';
...
const DUMMY_LOGIN_PASSWORD_HASH = '$argon2id$v=19$m=19456,t=2,p=1$aoZyQyB8eVS3LeO40/kqYQ$c0LhPvExxPfDSalntYXGc0qjrgJKJPj7Npz2xelpmnk';
...
router.post('/login', loginLimiter, validateRequest(z.object({ body: loginBodySchema, query: z.object({}).strict(), params: z.object({}).strict() })), asyncHandler(async (req, res) => {
    const username = normalizeUsername(req.body.username);
    const user = await prisma.user.findUnique({ where: { username } });
    const validPassword = await verifyPassword(user?.passwordHash ?? DUMMY_LOGIN_PASSWORD_HASH, req.body.password);
    const now = new Date();
...
router.get('/session', authenticate, validateRequest(emptyRequestSchema), asyncHandler(async (req, res) => {
    res.json({
        success: true,
        data: {
...
router.post('/password/change', authenticate, verifyCsrf, validateRequest(z.object({
...
router.post('/logout', authenticate, verifyCsrf, validateRequest(emptyRequestSchema), asyncHandler(async (req, res) => {
    const now = new Date();
    await prisma.$transaction(async (transaction) => {
        await transaction.authSession.updateMany({
            where: {
