...
import { authenticate, clearAuthCookies, csrfCookieOptions, CSRF_COOKIE_NAME, sessionCookieOptions, verifyCsrf, } from '../middlewares/auth.middleware.js';
...
router.post('/login', loginLimiter, validateRequest(z.object({ body: loginBodySchema, query: z.object({}), params: z.object({}) })), asyncHandler(async (req, res) => {
    ...
    if (user.status !== 'ACTIVE')
        throw new AppError('Bu hesap devre dışı.', 403);
...
router.post('/password/change', authenticate, verifyCsrf, validateRequest(z.object({ body: passwordChangeBodySchema, query: z.object({}), params: z.object({}) })), asyncHandler(async (req, res) => {
...
router.post('/logout', authenticate, verifyCsrf, asyncHandler(async (req, res) => {
