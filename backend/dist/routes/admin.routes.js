...
import { approveBookingApplication, createAudit, createBookingApplication, createUniqueCustomerUsername, rejectBookingApplication, retryUsernameConflict, } from '../services/booking.service.js';
import { AppError } from '../utils/appError.js';
import { asyncHandler } from '../utils/asyncHandler.js';
import { decryptValue, encryptValue, hashPassword } from '../utils/crypto.js';
import { assertGoogleDriveUrl, addCalendarDays, atIstanbulTime, createTemporaryPasswordExpiry, createWeddingRange, deliveryEncryptionAad, getIstanbulDate, messageSecretEncryptionAad, normalizePhone, randomTemporaryPassword, } from '../utils/domain.js';
const router = Router();
...
const isPrismaError = (error, code) => error instanceof Prisma.PrismaClientKnownRequestError && error.code === code;
const throwCatalogError = (error) => {
...
router.get('/booking-applications', validateRequest(z.object({ body: emptyBody, query: bookingQuerySchema, params: z.object({}) })), asyncHandler(async (req, res) => {
    ...
    const safeApplications = applications.map(({ idempotencyKey: _key, idempotencyFingerprint: _fingerprint, ...application }) => application);
...
router.get('/booking-applications/:id', validateRequest(uuidRequest), asyncHandler(async (req, res) => {
    ...
    const { idempotencyKey: _key, idempotencyFingerprint: _fingerprint, ...safeApplication } = application;
...
router.patch('/weddings/:id', verifyCsrf, validateRequest(z.object({
    ...
    const now = new Date();
    const temporaryPasswordExpiresAt = createTemporaryPasswordExpiry(env.TEMPORARY_PASSWORD_TTL_HOURS, activationAt > now ? activationAt : now);
    if (regenerateCredentials) {
        const temporaryPassword = randomTemporaryPassword();
        nextPasswordHash = await hashPassword(temporaryPassword);
        encryptedPassword = encryptValue(temporaryPassword, messageSecretEncryptionAad(wedding.id, 'ACCOUNT_ACTIVATION'));
    }
    const updateWedding = () => prisma.$transaction(async (transaction) => {
    ...
    const updated = regenerateCredentials
        ? await retryUsernameConflict(() => createUniqueCustomerUsername(req.body.brideLastName, req.body.groomLastName), async (username) => {
...
router.post('/deliveries/:id/deliver', verifyCsrf, validateRequest(uuidRequest), asyncHandler(async (req, res) => {
    ...
    const updated = await prisma.$transaction(async (transaction) => {
      ...
      `;
        ...
        const recipientPhone = currentWedding.primaryContact === 'GELIN'
...
const catalogRoutes = (path, schema) => {
    ...
    router.patch(`/${path}/:id`, verifyCsrf, validateRequest(z.object({ body: partialSchema, query: emptyQuery, params: uuidParamsSchema })), asyncHandler(async (req, res) => {
        let row;
        try {
            row = await prisma.$transaction(async (transaction) => {
                const updated = path === 'packages'
    ...
    router.delete(`/${path}/:id`, verifyCsrf, validateRequest(uuidRequest), asyncHandler(async (req, res) => {
        ...
        try {
            row = await prisma.$transaction(async (transaction) => {
                const deactivated = path === 'packages'
                    ? await transaction.package.update({
                        where: { id: req.params.id },
                        data: { isActive: false },
                    })
...
router.post('/message-tasks/:id/mark-sent', verifyCsrf, validateRequest(markSentRequest), asyncHandler(async (req, res) => {
...
router.post('/customers/:id/reset-password', verifyCsrf, validateRequest(uuidRequest), asyncHandler(async (req, res) => {
    ...
    const now = new Date();
    const temporaryPasswordExpiresAt = createTemporaryPasswordExpiry(env.TEMPORARY_PASSWORD_TTL_HOURS, user.activeAt !== null && user.activeAt > now ? user.activeAt : now);
    const encrypted = encryptValue(password, messageSecretEncryptionAad(user.customerWedding.id, 'PASSWORD_RESET'));
    const task = await prisma.$transaction(async (transaction) => {
        await transaction.$queryRaw `
        SELECT "id" FROM "weddings"
